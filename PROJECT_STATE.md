...
- Session update (2026-04-07, image-section follow-up):
  ...
  - Extended the per-image averaged steering notebook cell so it now also renders a representative single-trajectory montage for the selected image in both `dog->cat` and `dog->dog`, using the same visual style as the older single-trajectory steering figures.
