...
def process_single_image(image_name, args, classifier, preprocess, activations, activations_mean, activations_var, do_whitening):
    """
    Runs analysis for a specific image folder.
    """
    logger.log(f"--- Processing Image: {image_name} ---")
    
    # Construct output directory for this specific image
    out_dir = os.path.join(args.output_base, args.classifier_name, image_name, f"noise_{args.noise_step}")
    os.makedirs(out_dir, exist_ok=True)
    
    # Find trajectories
    base_search_path = os.path.join(args.data_dir, image_name, f"noise_step_{args.noise_step}", "trajectory_*")
    trajectory_dirs = sorted(glob.glob(base_search_path))
    
    if not trajectory_dirs:
        logger.log(f"No trajectories found for {image_name} at noise {args.noise_step}. Skipping.")
        return
    ...
    # --- Step 0 (Reference) ---
    batch_0, valid_indices_0 = load_batch_for_step(trajectory_dirs, 0, dist_util.dev())
    ...
    for k in range(1, max_steps + 1):
        ...
        batch_k, valid_indices_k = load_batch_for_step(valid_trajectory_dirs, k, dist_util.dev())
...
def main():
    ...
    for i, img_name in enumerate(image_names):
        process_single_image(
            img_name, args, classifier, preprocess, activations, 
            activations_mean, activations_var, do_whitening
        )
...
def create_argparser():
    defaults = dict(
        ...
        stats_dir=None, # Argument to point to the folder containing stats
    )
