...
# --- CUSTOM LOADER FOR SEQUENTIAL DEBUG DATA ---
def yield_sequential_debug_data(base_path, time_step, batch_size, device):
    """
    Scans the 'sequential_uturns_debug' folder structure.
    Finds: noise_step_{time_step} -> trajectory_X -> uturn_000 (Start) vs uturn_001 (Final).
    Yields batches of (start_tensor, final_tensor) in [-1, 1] range.
    """
    pairs = []
    
    # 1. Find all image folders (e.g. ILSVRC...) inside base_path
    image_folders = sorted(glob.glob(os.path.join(base_path, "*")))
    
    for img_folder in image_folders:
        if not os.path.isdir(img_folder): continue
        
        # 2. Look for the specific noise folder
        noise_folder = os.path.join(img_folder, f"noise_step_{time_step}")
        if not os.path.exists(noise_folder):
            continue
            
        # 3. Look for trajectories inside
        traj_folders = sorted(glob.glob(os.path.join(noise_folder, "trajectory_*")))
        
        for traj in traj_folders:
            # Try JPEG first, then PNG
            start_path = os.path.join(traj, "uturn_000.jpeg")
            if not os.path.exists(start_path): start_path = os.path.join(traj, "uturn_000.png")
            
            # For a single U-turn experiment, the result is uturn_001
            final_path = os.path.join(traj, "uturn_001.jpeg")
            if not os.path.exists(final_path): final_path = os.path.join(traj, "uturn_001.png")
            
            if os.path.exists(start_path) and os.path.exists(final_path):
                pairs.append((start_path, final_path))

    logger.log(f"Found {len(pairs)} pairs for noise step {time_step}")
    ...
    for i in range(0, len(pairs), batch_size):
        ...
        start_tensors = [process_img(p[0]) for p in batch_pairs]
        final_tensors = [process_img(p[1]) for p in batch_pairs]
...
def main():
    ...
    for time_step in args.time_series:
        ...
        data_loader = yield_sequential_debug_data(
            args.results_dir, 
            time_step, 
            args.batch_size, 
            dist_util.dev()
...
def create_argparser():
    defaults = dict(
        ...
        batch_size=32,
        image_size=256,
        # Point to your DEBUG folder
        results_dir="/work/pcsl/Noam/sequential_diffusion/results/sequential_uturns_debug",
        output_base=os.path.join(os.getcwd(), "classifier_statistics_debug"),
    )
