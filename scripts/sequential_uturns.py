...
Configured for 256x256 ImageNet model with 250-step respacing.
"""
...
def perform_single_uturn(model, diffusion, start_tensor, noise_step, device):
    ...
    with th.no_grad():
        ...
        # With SpacedDiffusion (respacing="250"), noise_step refers to the index 0..249.
        # We pass this index directly to q_sample.
        ...
        # We iterate backwards from (noise_step - 1) down to 0
        img = noisy_image
        indices = list(range(noise_step))[::-1]
        
        for i in indices:
            t = th.tensor([i] * img.shape[0], device=device)
            # p_sample handles the mapping from spaced index 'i' to original index internally
...
def create_argparser():
    ...
    defaults.update(dict(
        ...
        # --- CONFIGURATION TO MATCH ORIGINAL EXPERIMENT ---
        image_size=256,
        class_cond=False,
        learn_sigma=True,
        diffusion_steps=1000,   # Model was trained with 1000 steps
        noise_schedule="linear",
        timestep_respacing="", # Revert to using the full 1000 steps
        # timestep_respacing="250", # Sampling uses 250 steps (matches 'working' code)
        use_kl=False,
